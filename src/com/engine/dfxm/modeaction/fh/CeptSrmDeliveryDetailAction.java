...
*/
public class CeptSrmDeliveryDetailAction extends AbstractModeExpandJavaCodeNew {
	private Logger log= LoggerFactory.getLogger();


	/**
	 *
	 * 1.发货单保存时，校验明细，未创建检查卡的零件，创建对应的检查卡内容
	 * 2.已创建检查卡的，跳过创建
	 * 3.用户在发货单详情tab页去填写每份零件对应的检查卡
	 * 执行模块扩展动作
	 * @param param
	 *  param包含(但不限于)以下数据
	 *  user 当前用户
		...
		try {
			...
			if(requestInfo!=null){
				...
				if(billid>0&&modeid>0){
					boolean hasjck=false;
					if(hasjck){
						Map<String, Object> reMapJCK=doSend360JCK("360JCK",billid+"","","检查卡(360)");
						log.info("发货检查卡接口返回"+reMapJCK);
						if((boolean) reMapJCK.get("status")||true){
							Map<String, Object> reMap360=doSend360("360FH",billid+"","","发货(360)");
							log.info("发货接口返回"+reMap360);
							...
							result.put("flag", "false");
							result.put("errmsg",(String) reMapJCK.get("msg"));
						}
					}else{
						Map<String, Object> reMap360=doSend360("360FH",billid+"","","发货(360)");
						log.info("发货接口返回"+reMap360);
	...
	private Map<String, Object> doSend360JCK(String operateCode, String ids, String type, String doMsg) {
		...
		try {
			...
			if (rs.next()){
				...
				//data.put("CHECK_CARD_DETAIL",detaildata);



				//JSONObject d=JSONObject.fromObject("{\n" +
				//		"    \"CHECK_CARD_MAIN\": {\n" +
				//		"        \"DELIVERY_CODE\": \"1\",\n" +
				//		"        \"DELIVERY_LINECODE\": \"2\",\n" +
				//		"        \"MATTER_CODE\": \"1\",\n" +
				...
				//		"            \"LINE_NUMBER\": \"3\",\n" +
				//		"            \"INSPECT_ITEM\": \"4\",\n" +
				//		"            \"TECHNICAL_REQUIRE\": \"20\",\n" +
				//		"            \"IMPORTANCE\": \"3\",\n" +
				//		"            \"KEY_INSPECT_FLAG\": \"Y\",\n" +
				//		"            \"INCOME_INSPECT_FLAG\": \"Y\",\n" +
				//		"            \"NAME_MODEL\": \"3\",\n" +
				//		"            \"UNCERTAINLY\": \"4\",\n" +
				//		"            \"SUPPLIER_INSPECT1\": \"20\",\n" +
				//		"            \"SUPPLIER_INSPECT2\": \"3\",\n" +
				//		"            \"SUPPLIER_INSPECT3\": \"4\",\n" +
				//		"            \"SUPPLIER_INSPECT4\": \"20\",\n" +
				//		"            \"SUPPLIER_INSPECT5\": \"3\",\n" +
				//		"            \"SUPPLIER_QUAL_INSPECT\": \"4\"\n" +
				//		"        }\n" +
				//		"    ]\n" +
				//		"}");
				//data=d;
