package com.engine.dfxm.cmd.thd;
...
*/
public class AcceptGoodsRuslut extends AbstractCommonCommand<Map<String, Object>> {
    private Logger log= LoggerFactory.getLogger();

    public AcceptGoodsRuslut(Map<String, Object> params, User user) {
        this.user = user;
        this.params = params;
    }
    ...
    public Map<String, Object> execute(CommandContext commandContext) {
        ...
        Map<String, Object> reMap360=doSend3601("360THJGCZ",ids,type,doMsg+"(360)");
        return reMap360;
    }
    ...
    private Map<String, Object> doSend360(String operateCode, String ids, String type, String doMsg) {
        ...
        try {
            ...
            if(dataArr.size()>0){
                ...
                //    reMap.put("msg",doMsg+"失败,暂停第三方接口");
                //    return reMap;
                //}
                ...
                if(response.isOk()){
                    ...
                    if("200".equals(retJson.getString("code"))){
                        ...
                        if("S".equals(retdata.getString("MESSAGE_TYPE"))){
                            calculate(ids,pmObj);
                            reMap.put("status",true);
                            //发货成功，需要更新发货表的发货状态和订单表的待发货数量和已发货数量
        ...
        try {
            ...
            if (rs.next()){
                ...
                if(allDetailDataArr.size()>0){
                    ...
                    reMap.put("status",false);
                    reMap.put("msg","退货明细数据不存在，请确认数据是否正确！");
                }
            }else{
                reMap.put("status",false);
                reMap.put("msg","退货主表数据不存在，请确认数据是否正确！");
            }
        }catch (Exception e){
            reMap.put("status",false);
            reMap.put("msg",doMsg+"失败,第三方接口异常："+e);
