 ...
 * @author Alex.Du
 */
public class SynOldPIHornetQQuartz  extends BaseCronJob {
    private BaseBean baseBean = new BaseBean();

    @Override
    public void execute() {
        baseBean.writeLog("[SynOldPIHornetQQuartz]**************************开始启动PI的HonnetQ监听*********************");
        ...
        try {
            ...
            providerURL = "jnp://" + baseBean.getPropValue("integration_address_config","old_pi_hornet_ip");

            baseBean.writeLog("[SynOldPIHornetQQuartz]providerURL:"+providerURL);
...
//            QueueReceiver receiver = session.createReceiver(que);
            ...
            baseBean.writeLog("[SynOldPIHornetQQuartz]队列连接创建成功");
            ...
            while(true) {
                ...
                baseBean.writeLog("[SynOldPIHornetQQuartz]获取到的消息对象objMessage为:" + objMessage);

                if (objMessage == null) {
                    baseBean.writeLog("[SynOldPIHornetQQuartz]未获取到消息数据");
                    return;
                }

                //收到消息
                baseBean.writeLog("[SynOldPIHornetQQuartz]收到消息数据");

                String contentXML = null;
                byte[] contentByte = null;

                try {
                    baseBean.writeLog("[SynOldPIHornetQQuartz]即将进行数据对象转换");
                    if (objMessage.getObject() instanceof byte[]) {
                        baseBean.writeLog("[SynOldPIHornetQQuartz]数据类型是byte[]");
                        contentByte = (byte[]) objMessage.getObject();
                        baseBean.writeLog("[SynOldPIHornetQQuartz]获取到的内容为：" + new String(contentByte, "UTF-8"));
                        contentXML = new String(contentByte, "UTF-8");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    baseBean.writeLog("[SynOldPIHornetQQuartz]解析PT消息内容出现异常:" + e.getMessage());
                }
                ...
                if (contentXML != null && !contentXML.trim().equals("")) {
                    ...
                    try {
                        ...
                        //获取xml的节点内容
                        baseBean.writeLog("[SynOldPIHornetQQuartz]tradeCode:" + tradeCode.getTextTrim());
                        //获取数据节点集合
                        ...
                        for (int i = 0; i < rowList.size(); i++) {
                            ...
                            if (tradeCode.getTextTrim().equals("PT-COS-01")) {
                                ...
                                try {
                                    ...
                                    e.printStackTrace();
                                    baseBean.writeLog("[SynOldPIHornetQQuartz]存储同步的人员信息出现异常：" + e.getMessage());
                                    continue;
                                }
                                ...
                                try {
                                    ...
                                    e.printStackTrace();
                                    baseBean.writeLog("[SynOldPIHornetQQuartz]存储同步的组织信息出现异常：" + e.getMessage());
                                    continue;
                                }
                            }
                        }

                        baseBean.writeLog("[SynOldPIHornetQQuartz]数据同步到中间表结束，开始执行同步数据处理");
                        new SynEHROrgHrmQuartz().execute();
                        baseBean.writeLog("[SynOldPIHornetQQuartz]执行同步数据处理结束");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            e.printStackTrace();
            baseBean.writeLog("[SynOldPIHornetQQuartz]启动PI的HonnetQ监听出现异常:" + e.getMessage());
            baseBean.writeLog("[SynOldPIHornetQQuartz]**************************启动PI的HonnetQ监听失败*********************");
        }finally {
            try {
                if (session != null) {
                    session.close();
    ...
    private void saveSynOrgInfo(Element row) throws Exception {
        new BaseBean().writeLog("[SynOldPIHornetQQuartz]进入存储同步组织信息的方法");
        RecordSet rs =null;
        try {
            new BaseBean().writeLog("[SynOldPIHornetQQuartz]开始实例化RecordSet");
            rs = new RecordSet();
            new BaseBean().writeLog("[SynOldPIHornetQQuartz]RecordSet实例化完毕");
            // 通过部门ID查找该部门是否已经存在
            new BaseBean().writeLog("[SynOldPIHornetQQuartz]通过部门ID查找部门是否存在的SQL为：select id from uf_ehr_org_data where ORG_ID = '"
                    + row.attribute("ORG_ID").getValue() + "'");
            rs.execute("select id from uf_ehr_org_data where ORG_ID = '"
                    + row.attribute("ORG_ID").getValue() + "'");
            new BaseBean().writeLog("[SynOldPIHornetQQuartz]通过部门id查找到"+rs.getCounts()+"条");

            if (rs.next()) {
                new BaseBean().writeLog("[SynOldPIHornetQQuartz]进入修改部门信息");
                new BaseBean().writeLog("[SynOldPIHornetQQuartz]当前部门存在，更新此部门的SQL为：update uf_ehr_org_data set ORG_ID='" + row.attribute("ORG_ID").getValue()
                ...
                new BaseBean().writeLog("[SynOldPIHornetQQuartz]进入新增部门信息");
                new BaseBean().writeLog("[SynOldPIHornetQQuartz]当前部门不存在，新建此部门的SQL为：insert into uf_ehr_org_data(ORG_ID,ORG_CODE,ORG_NAME,PARENT_ORG_ID,ORG_ADDRESS,ZIP_CODE,CONTACT_PERSON,CONTACT_PERSON_TEL,EMAIL,WEB_URL,MEMO,ORG_STATUS,ORG_CLASS,ACT,IS_SYN,LAST_UPDATE_TIME) values('" + row.attribute("ORG_ID").getValue()
    ...
    private void saveSynHrmInfo(Element row) throws Exception {
        new BaseBean().writeLog("[SynOldPIHornetQQuartz]进入存储同步的人员信息");
        ...
        try {
            ...
            if (rs.next()) {
                new BaseBean().writeLog("[SynOldPIHornetQQuartz]进入更新人员的语句");
                new BaseBean().writeLog("[SynOldPIHornetQQuartz]进入更新人员的语句：update uf_ehr_hrm_data set OPERATER_ID='" + row.attribute("OPERATER_ID").getValue()
