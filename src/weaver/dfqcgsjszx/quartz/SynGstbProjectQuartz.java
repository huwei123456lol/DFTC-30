...
public class SynGstbProjectQuartz extends BaseCronJob {
    ...
    public void execute() {
        ...
        String[] tree_id = new weaver.general.BaseBean().getPropValue("gstb_project_root","root").split(";");
        new BaseBean().writeLog("==============SynGstbProjectQuartz_tree_id  ["+ Arrays.toString(tree_id) +"]==============");
        IGatewayServiceProxy proxy = new IGatewayServiceProxy();
        RecordSet rs = new RecordSet();
        for (String treeID:tree_id) {
            new BaseBean().writeLog("==============SynGstbProjectQuartz_tree_id   " + treeID + "==============");
    ...
    public void synProjectList(String parentTreeID,IGatewayServiceProxy proxy,RecordSet rs) {
        ...
        try {
            ...
            new BaseBean().writeLog("==============SynGstbProjectQuartz_para   " + para + "==============");
            //从接口获取返回数据
            String result = proxy.queryProjectList(para);
            new BaseBean().writeLog("==============SynGstbProjectQuartz_result " + result + "==============");
            ...
            for (int i = 0; i < data.length(); i++) {
                ...
                //fqbm,sqr,zzsqr,sqbm,sqrq,zzrq,sqrgw,zzmm,syqx,zzsm,djbh
