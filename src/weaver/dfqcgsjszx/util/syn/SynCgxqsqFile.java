...
public class SynCgxqsqFile {
    ...
    public void start(){
        ...
        //查询当前已同步了多少篇文档
        recordSet.execute("select max(keyword+0) as maxKeyword from docdetail where seccategory=7546");
        String maxKeyword = "";
        if(recordSet.next()){
        ...
        if(maxKeyword!=null&&!maxKeyword.trim().equals("")){
            appendWhere = " and attid >"+maxKeyword;
        }
        ...
        try{
            ...
            rs = statement.executeQuery("select * from OLD_CGXQ_FJ where fjcontent is not null"+appendWhere+" order by attid");
            while(rs.next()){
                baseBean.writeLog("当前处理的attid："+rs.getString("attid"));

                recordSet.execute("select * from docdetail where keyword='"+rs.getString("attid")+"' and seccategory=7546");
                if(recordSet.next()){
                    baseBean.writeLog("attid为【"+rs.getString("attid")+"】的附件已经存在，将跳过该条数据的处理");
                    continue;
                }

                String sqdbh = rs.getString("SQDBH");//申请单编号
                baseBean.writeLog("当前处理的申请单编号为：" + sqdbh);
                String fjName = rs.getString("FJNAME");//附件名称
                baseBean.writeLog("附件名称为：" + fjName);
                Blob fjContent = rs.getBlob("FJCONTENT");//附件内容，字节输入流
                ...
                if (!file.exists()) {
                    try {
                        ...
                        e.printStackTrace();
                        baseBean.writeLog("创建申请单编号为：" + sqdbh + "的采购申请单附件文件时出现异常(exception:+" + e.getMessage() + ")");
                        continue;
                    }
                }
                ...
                try {
                    ...
                    // 创建docdetail
                    int docid = createDocDetail(7546, file.getName(), 1);
                    // 创建docimagefile
                    createDocImageFile(docid, imagefileid, file.getName());
                    // 创建文档共享
                    createDocShare(7546, 1, docid);

                    recordSet.execute("update docdetail set keyword='" + rs.getString("attid") + "' where id='" + docid + "'");
                    baseBean.writeLog("修改文档keyword的sql语句：update docdetail set keyword='" + rs.getString("attid") + "' where id='" + docid + "'");

                    new BaseBean().writeLog("存储到ecology系统中的文件ID为：" + docid);

                    recordSet.execute("update uf_cgxqjbxxb set xgfj='" + docid + "'||','||xgfj where cgxqbh='" + sqdbh + "'");
                    baseBean.writeLog("执行的更新语句：update uf_cgxqjbxxb set xgfj='" + docid + "'||','||xgfj where cgxqbh='" + sqdbh + "'");

                    recordSet.execute("select cgxqlc from uf_cgxqjbxxb where cgxqbh='" + sqdbh + "'");
                    while(recordSet.next()){
                        baseBean.writeLog("查询到的cgxqlc字段值为：'"+recordSet.getString("cgxqlc")+"'");
                        if(null!=recordSet.getString("cgxqlc")&&!recordSet.getString("cgxqlc").trim().equals("")){
                            recordSet.execute("update formtable_main_168 set xgfj='" + docid + "'||','||xgfj where requestid='"+recordSet.getString("cgxqlc")+"'");
                            baseBean.writeLog("执行的更新采购需求流程表的语句为：update formtable_main_168 set xgfj='" + docid + "'||','||xgfj where requestid='"+recordSet.getString("cgxqlc")+"'");
                        }
                    }
                }catch(Exception e){
                    e.printStackTrace();
                    baseBean.writeLog("同步采购需求申请的附件出现异常1："+e.getMessage());
                    continue;
                }finally {
                    if (os != null) {
                        try {
