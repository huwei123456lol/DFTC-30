 ...
 * @author Alex.Du
 */
public class SearchPayResultByZplyQuartz584 extends BaseCronJob {
    public void execute() {
        ...
        rs.execute("select fm.* from formtable_main_584 fm,workflow_requestbase wr where wr.currentnodetype=3 and (fm.fkzt is null or fm.fkzt='') and fm.requestid=wr.requestid and fm.sqsj>='2019-11-20'");
        ...
        while (rs.next()) {
            ...
            try {
                ...
                if(dataResult.getString("RET_CODE").equals("0")){
                    rs2.execute("update formtable_main_584 set fkzt='付款成功',fkfs='"+fkfs+"' where requestid='"+rs.getString("requestid")+"'");
                }else if(dataResult.getString("RET_CODE").equals("-1")){
                    rs2.execute("update formtable_main_584 set fkzt='付款失败',fkfs='"+fkfs+"' where requestid='"+rs.getString("requestid")+"'");
                }else if(dataResult.getString("RET_CODE").equals("-3")){
                    rs2.execute("update formtable_main_584 set fkzt='付款被退回（"+dataResult.getString("ERROR_MSG")+"）',fkfs='"+fkfs+"' where requestid='"+rs.getString("requestid")+"'");
                }else if(dataResult.getString("RET_CODE").equals("-4")){
                    rs2.execute("update formtable_main_584 set fkzt='票据已开',fkfs='"+fkfs+"' where requestid='"+rs.getString("requestid")+"'");
                }
            } catch (Throwable e) {
                e.printStackTrace();
