...
/**
 * 管网设备设施维护流程数据发送到NC的Action
 * 
 * @author Alex.Du
 * 
 */
public class GwsbsswhSendNCAction extends BaseAction {
	public String execute(RequestInfo requestInfo) {
		...
		log.writeLog("select ceah from uf_cea where id = '" + ceaID + "'");
		...
		log.writeLog("select departmentname from HrmDepartment where id = '"
				+ bmid + "'");
		...
		log.writeLog("select ncbm from uf_ncdygx where bm = '" + bmid + "'");
		...
		log.writeLog("select loginid,lastname from hrmresource where id = '"
				+ sqrId + "'");
		...
		log.writeLog("select selectname from Workflow_Selectitem where fieldid = 9934 and selectvalue = '"
				+ stateid + "'");
		...
		log.writeLog("select dq from uf_dq where id= '" + fsxbdqid + "'");
