 ...
 * 
 */
public class KwjdxqxSendNCAction extends BaseAction {
	public String execute(RequestInfo requestInfo) {
		...
		rs.execute("select ncepsfl from uf_ceaxmxl where id = '" + fsxbgcxmllID
				+ "'");
		if (rs.next()) {
			log.writeLog("111111111---");
			fsxbgcxmll = rs.getString("ncepsfl");// EPS分类编码
			log.writeLog("查询到的EPS分类编码为：'"+fsxbgcxmll+"'");
			if (fsxbgcxmll == null || fsxbgcxmll.trim().equals("")) {
				// 如果為空則不觸發NC接口
				log.writeLog("EPS分类编码为空");
